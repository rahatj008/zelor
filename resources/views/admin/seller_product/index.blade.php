                            ...
                            @forelse($products as $product)
                                <tr>
                                    ...
                                    <td data-label="{{translate('Info')}}">
                                        <!--<span>{{translate('Regular Price')}} : {{(short_amount($product->price))}} <br> {{translate('Discount Price')}} : {{(short_amount($product->discount))}}</span><br>-->
                                        <a href="{{route('admin.item.product.inhouse.bestsellingitem.status', $product->id)}}" class=" fs-15 link-{{$product->best_selling_item_status == '1' ? 'danger':'info'}}" data-bs-toggle="tooltip" data-bs-placement="top" title="{{translate('Best Selling Product')}}">{{translate($product->best_selling_item_status==1?'Best Selling Item - No':'Best Selling Item - Yes')}} <i class="las la-arrow-alt-circle-left"></i></a>
                                        <br>

                                        <a href="{{route('admin.item.product.inhouse.suggested.status', $product->id)}}" class=" fs-15 link-{{$product->is_suggested == 0 ? 'danger':'info'}}"  data-bs-toggle="tooltip" data-bs-placement="top" title="{{translate('Suggested Product')}}">{{translate($product->is_suggested == 0?'Suggested Item - No':'Suggested Item  - Yes')}} <i class="las la-arrow-alt-circle-left"></i></a>
