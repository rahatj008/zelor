    ...
    <div class="product-detail-middle">
        ...
        <div class="product-item-review">
            <div class="ratting mb-0">
               @php echo show_ratings($product->rating()->avg('rating')) @endphp <small class="text-white">({{$product->rating()->count()}} {{translate('Reviews')}})</small>
            </div>
            <small class="text-white">{{$product->order->count()}}
                {{translate("Orders")}}
            </small>
        </div>
        <div class="product-item-price  price-section">
            ...
            @if(count($product->campaigns) != 0 && $product->campaigns->first()->end_time > Carbon\Carbon::now()->toDateTimeString() &&   $product->campaigns->first()->status == '1')
                        ...
                        @if(($product->campaigns->first()->pivot->discount) == 0)
                              <span class="text-white">{{(short_amount($price ))}}</span>
                        @else
                    ...
                    @if(($product->discount_percentage) > 0)
                        ...
                        <del class="text-white"> {{short_amount($price)}}</del>
        ...
        <form class="attribute-options-form-{{$randNum}} quick-view-form">
            ...
            <div class="product-actions-type">
                ...
                <button data-product_id ="{{$product->id}}"  class="product-details-love-btn wishlistitem" style="background: linear-gradient(to bottom, #CD3C3E, #671E1F) !important;">
                    <i style="color:#fff !important;" class="@if(in_array($product->id,$wishedProducts))
                        fa-solid
                    @else
                        fa-regular
                    @endif fa-heart"></i>
                </button>
                <button class="product-details-love-btn comparelist wave-btn" data-product_id="{{$product->id}}" style="background: linear-gradient(to bottom, #CD3C3E, #671E1F) !important;"><i style="color:#fff !important;" class="fa-solid fa-code-compare"></i></button>
            </div>
        </form>
        <div class="product-detail-btn">
            <a href="javascript:void(0)" style="background: linear-gradient(to bottom, #CD3C3E, #671E1F) !important;" data-checkout = "yes" data-product_id = "{{$randNum}}" class="buy-now-btn quick-buy-btn addtocartbtn">
                <i style="color:#fff !important;" class="fa-solid fa-cart-plus fs-2 me-3"></i>{{translate("Buy Now")}}
            </a>
