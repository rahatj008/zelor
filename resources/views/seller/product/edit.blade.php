...
<script>
    ...
    //     console.log($('#updateproduct-form').serialize());
    //     $.ajax({
    //         headers: {
    //             'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    //         },
    ...
    // }
    function combinations() {
        $('#mail-composer').val(encodeURIComponent($('#mail-composer').val().replace(/<!DOCTYPE[^>]*>/gi, '').trim()));
        $('#short_description').val(encodeURIComponent($('#short_description').val().replace(/<!DOCTYPE[^>]*>/gi, '').trim()));

        const formData = $('#updateproduct-form').serialize();
